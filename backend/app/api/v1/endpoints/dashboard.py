from datetime import datetime
from pyexpat.errors import messages

from fastapi import APIRouter, HTTPException
from fastapi.params import Depends
from sqlalchemy.orm import Session

from app.crud.client import create_client
from app.crud.vehicle import create_vehicle
from app.dependencies.db import get_db
from app.dependencies.jwt import get_current_mechanic_id_from_cookie
from app.models.vehicles import Vehicles
from app.schemas.vehicle import VehicleCreate

router = APIRouter()

@router.post("/add_vehicle")
def add_vehicle(
        data: VehicleCreate,
        mechanic_id: int = Depends(get_current_mechanic_id_from_cookie),
        db: Session = Depends(get_db)
    ):
    print(data)
    if data.client_id:
        client_id = data.client_id
    elif data.client:
        client_id = create_client(db, data.client, mechanic_id)
    else:
        raise HTTPException(status_code=400, detail="Provide client_id or client_data")

    new_vehicle_data = {
        "mark": data.mark,
        "model": data.model,
        "vin": data.vin,
        "client_id": client_id,
        "last_view_data": datetime.utcnow(),
    }

    new_vehicle_id = create_vehicle(db, new_vehicle_data)

    return {"vehicle_id": new_vehicle_id}

@router.get("/get_recently_used_vehicles")
def get_recently_used_vehicles(
        mechanic_id: int = Depends(get_current_mechanic_id_from_cookie),
        db: Session = Depends(get_db)
    ):
    all_vehicles = db.query(Vehicles).order_by(Vehicles.last_view_data.desc()).limit(5).all()
    return {"vehicles": all_vehicles}

